...
- 컴퓨터 안에는 **비트**만이 존재함
  - 두 가지 가능한 상태에서 한 가지 상태를 선택하고 담아 놓을 수 있는 장소
  - 전류가 흐르거나 흐르지 않는 장소
...
- 어떤 사물이 다른 사물을 의미하도록 만들어 주는 방법
  - 비트 여러 개를 이용해 **상태를 표현하고, 각각에 대응되는 의미 목록**을 만들 수 있음
...
- 모든 장치의 입력 제어단자 s를 하나로 묶어놓음 : 메모리 8개가 동시에 비트의 상태를 기록함
...
- 버스에 연결된 레지스터들의 데이터 전송
  ...
  - R1의 데이터가 R2로 복사 완료됨
- 2개 이상의 레지스터가 동시에 데이터를 출력하면 안 됨
  - 충돌이 생겨서 데이터 일관성이 깨짐 
...
- 한 번에 1바이트씩 처리하기 위해서는 비트가 아니라 바이트를 처리할 게이트가 필요함
  - 연산을 이용함
...
- 데이터 수준에서 산술이나 논리를 계산하는 장치 7개를 합친 장치
- 버스 입력 2개, 입력 비트 4개(**op code** + 자리 올림 입력), 버스 출력 1개, 출력 비트 4개로 구성됨
- 3*8 디코더를 이용하여 하나의 장치를 활성화하고 그 장치의 연산 결과를 출력함
  - 디코더 입력을 op code라고 함
...
- 각 장치의(램, 레지스터, ALU) 제어 비트를 설정함으로써 작동시킴 
...
- 스스로 0과 1을 주기적으로 오가는 비트
  - 아무런 입력 없이 출력 비트를 생성함
  - 출력 비트를 클록 비트라 함
  - **클록의 반복 속도에 의해 컴퓨터 전체의 동작 속도가 결정**됨
- NOT게이트의 출력을 다시 입력으로 보냄
  - 전류가 전선을 타고 돌아가는 시간*2를 주기로 하는 사이클이 됨
- 클록의 이용
  - 바이트 전송시 출력 허가 시점과 입력 허가 시점의 컨트롤 : 출발지 레지스터의 출력 제어 비트를 0으로 만들기 전에 목적지 레지스터의 입력 제어 비트를 0으로 만들어야 함
  - 원본 클록과 지연된 클록을 OR과 AND로 연결하여 clk e, clk s출력을 생성함
...
- 자동으로 일련의 여러 명령어를 수행할 수 있어야 함
  - 여러 명령어의 처리가 필요함
    - 명령어 코드와 스테퍼 사이에 새로운 게이트를 추가하고 배선함
  - 여러 명령어 중에 하나를 선택하여 작업해야 함
    - **명령어 집합**을 구현함
- 프로그램은 명령어 가져오기 + 실행하기 단 2가지의 조합으로 이루어짐



명령어 집합

- 명령어 집합은 메인 메모리에 위치 시킴
  - 특정한 비트의 조합과 명령어를 매핑시키는 테이블
