...
- 관계형 데이터베이스
  ...
  - 단점
    - 빅데이터 처리가 어려움 : 데이터를 처리하는 데 필요한 비용의 증가
    - 관계형 모델과 메모리 내 데이터 구조 간 데이터 불일치 : **ORM**을 통해 해결할 수 있음
- 비관계형 데이터베이스
  - 장점
    - 확장성 : **JOIN 처리가 없음** → 스케일 아웃을 통한 노드 확장이 용이함
...
- 클러스터링이란?
  - 여러 개의 DB를 **수평적인 구조로 구축**하는 방식
    - Fail Over한 시스템임 : 1개의 노드가 죽어도 다른 노드가 살아있어 장애없이 운영할 수 있음
...
- 추가적인 쓰기 작업과 저장공간을 활용하여 DB테이블의 **검색 속도를 향상시키기 위한 자료구조**
  - 특정 column에 대해 해당 컬럼의 데이터와 인덱스 주소를 매핑해 따로 저장함
    - Search Key와 해당 레코드의 주소를 나타내는 pointer로 구성됨
...
- 위와 같은 Multilevel Index의 경우 **상위 레벨(Outer Index)은 메인 메모리에 저장**
  - 물론 모든 인덱스가 메모리에 올라가면 좋지만 공간이 부족하면 하위 레벨은 디스크로 내려감
...
- INSERT, UPDATE, DELETE 등 데이터 수정이 적게 일어나는 경우
  - 애초에 수정이 있을 때마다 인덱스도 수정해줘야 하기 때문에 당연히 두배로 손해이다
  - INSERT
