...
- 하나의 작업이 다른 작업을 막는지 안 막는지
  ...
  - 블로킹: 다른 주체의 작업의 시작부터 끝까지 기다렸다가 다시 자신의 작업을 시작
...
- 주소가 결정되는 시기에 따른 분류
  ...
  - 로드 타임 바인딩
    ...
    - **loader**에 의해 프로그램이 종료될 때까지 메모리 위치 고정
    - swap out되었다가 다시 메모리에 올라갈 때도 같은 위치로 올라가야 함
    - 로더는 프로그램을 메모리에 적재시키는 코드
...
- Memory Management Unit
  - 논리적 주소를 물리적 주소로 매핑해주는 하드웨어
    - CPU가 논리적 주소를 이용해서 참조하려 함
    - MMU가 논리 주소값에 **기준 레지스터** 값을 더해 물리 주소값을 얻음
