...
- 스레싱 방지 알고리즘
  ...
  - 워킹셋 알고리즘
    ...
    - **워킹셋**을 구성하는 페이지들이 한꺼번에 메모리에 올라갈 수 있는 경우에만 해당 프로세스에 메모리를 할당
  ...
  - 페이지 폴트 빈도 알고리즘
    ...
    ![pff](https://lh3.googleusercontent.com/proxy/-LRSm3jrPf1OIEXbXV1_GX3boobUvB8cP2vzGk5QEWzsg-10G_t5MNEgv4qAq_ux3JuaOjbNzXCctL6VNVhBFy1jSk_ip5loq6sFywnxyR_CLwbYhA)
...
- 데이터 블록: 실제 데이터
  ...
  - blocks에 12개 블록을 저장함: 48KB는 작기 때문에 indirect block을 이용
    ...
    - single indirect는 4KB의 블록을 가리키고, 해당 블록은 4byte의 **direct block pointer 1024개로 구성되어 각각이 블록을 가리킴**
...
- 여러가지 **실행환경을 통채로 백업** 가능: 실무에 많이 사용됨
