...
- 다중 프로세스
  ...
  - 멀티 프로그래밍
    - 단일 CPU에서 여러 프로그램(프로세서)이 **동시에 실행**될 수 있도록 하는 것
...
- 프로세스의 상태는 크게 실행(running), 준비(ready), 봉쇄(blocked)
  - 하나의 프로세서에서 현재 **실행 중인 프로세스는 1개 뿐**
    - 코어가 아니라 프로세서인가?
    - 프로세서 : CPU를 포함하는 개념(명령어를 처리하는 논리회로)인데 CPU랑 같다고 봐도 무방
...
- lock의 문제점
  ...
  - **Deadlock** : 스레드 간에 대기 상태가 종료되지 않아 무한정 대기하는 것
...
- 주소가 결정되는 시기에 따른 분류
  - 컴파일 타임 바인딩
    ...
    - 현재는 잘 사용되지 않음
  - 로드 타임 바인딩
    - 프로그램의 **실행이 시작될 때** 주소 결정 : 프로세스가 run 될 때를 말하는 건가?
