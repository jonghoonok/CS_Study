...
- **시스템 콜** or 시스템 호출 인터페이스
  ...
  - 시스템 콜 정의
    ...
    - POSIX: 이식 가능 운영 체제 인터페이스를 뜻하며 서로 다른 유닉스 OS의 공통 API를 정리한 규격
...
- 사용자 모드: 일반 사용
  - 제한적인 명령만을 수행할 수 있음
  - **다른 프로그램의 메모리 영역 등을 침범하지 않게**끔 CPU 내부에 mode bit를 두어 0이면 커널모드로서 모든 명령 수행, 1이면 제한된 명령만 수행할 수있게 함
...
- 다중 프로세스
  ...
  - 멀티(병렬) 프로세싱
    - 여러 CPU가 하나의 프로그램을 병렬로 실행해서 속도 극대화
  - 멀티 프로그래밍
...
- 스왑: 프로세스가 메모리를 당분간 획득할 가능성이 없을 때(봉쇄 상태) 디스크로 내림
...
- 문맥: 프로세스가 어떤 상태에서 수행되고 있는지 정확히 규명하기 위해 필요한 정보
  ...
  - 컨텍스트 스위치가 일어날 때, **dispatcher**가 context를(PC, SP) 해당 프로세스의 **PCB**에 저장해두고 다른 프로세스 실행
...
- **Inter Process Communication**
  ...
  - 공유 메모리 방식
    ...
    - 동기화 문제 발생 가능
- 동기화 문제란?
  - 공유 자원 무결성에 문제가 생겨 비정상적인 동작이 발생하는 것
...
- 프로세스 내부의 **CPU 수행 단위**
  ...
  - 예를 들어 웹브라우저는 웹페이지를 로딩할 때 여러 스레드를 이용해 한 스레드가 I/O작업을 하는 중에 다른 스레드가 이미 읽어온 페이지를 로딩하여 응답성을 향상함
- 스레드 구조
...
- lock의 문제점
  ...
  - **Deadlock** 
    ...
    - 회피: 발생 조건은 그냥 두고, **은행원 알고리즘**(자원 할당해도 되는지 판단) 등으로 회피 
    - 탐지&회복: 교착이 발생하면 탐지한 후 회복
    - 무시: 발생확률이 낮다면 그냥 무시함
  - **Starvation**
...
- **장기 스케줄러(job scheduler)**: 어떤 프로세서를 준비 큐에 넣을 것인가?
...
- **단기 스케줄러**: 준비 큐의 어떤 프로세스를 실행할 것인가?
  ...
  - 스케줄링 방식
    - 비선점형: 프로세스가 스스로 CPU를 반납하기 전까지 뺏기지 않는 것
      - 실행 중인 프로세스가 I/O 요청에 의해 봉쇄되는 경우, 실행중인 프로세스의 종료
...
- 사용자 관점
  ...
  - 대기시간 waiting time
    - CPU버스트 중 프로세스가 준비 큐에서 기다린 시간의 합
...
- 현대 컴퓨터 시스템에서 OS는 인터럽트가 발생할 때만 실행됨
  ...
  - OS는 인터럽트가 발생했을 때만 CPU의 제어권을 획득
...
- CPU 관점에서
  ...
  - 해당하는 **인터럽트 처리루틴**을 찾아서 작업 수행
    - **인터럽트 벡터**: 인터럽트 종류마다 번호를 정해 번호에 따라 처리할 코드가 위치한 부분을 가리키는 자료구조
      - x86에서는 IDT(Interrupter Discriptor Table)을 사용
...
[참고](https://sjh836.tistory.com/109)
...
- **기준 레지스터, 한계 레지스터**를 이용하여 메모리를 보호
  - 기준 레지스터: 프로그램의접근 가능한 메모리상의 가장 작은 주소
...
> 프로세스에 충분한 메모리를 할당해주기 위해 메모리가 실제 물리적 메모리보다 더 많아보이게 함
...
- **캐싱 기법**
  ...
  - 용량이 적고 빠른 저장장치를 이용해 느린 저장장치의 성능을 향상시키는 기법
    - 느린 저장장치에 있는 내용 중 당장 사용되거나 빈번히 사용될 정보를 선별하여 빠른 저장장치에 저장하여 속도를 완충시킴
