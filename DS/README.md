...
- 중복을 허용하지 않음
- 노드의 왼쪽 서브트리에는 노드보다 작거나 같은 값, 오른쪽 서브트리에는 크거나 같은 값이 저장됨
...
- 균형인수가 +2인 노드를(W) 균형인수가 +1인 노드의(X) 오른쪽 자식 노드가 되도록 함
  - 이 때 X노드에 오른쪽 서브트리가 존재한다면 W의 왼쪽 서브트리가 되도록 함
...
- BST에 새 노드를 삽입하는 것과 동일하게 하고 새 노드 N을 레드로 함
  ...
  - N의 부모 노드 P가 블랙인 경우
    - 모든 조건을 만족하므로 pass
  - N의 부모 노드 P가 레드고, P의 형제 노드 U가 블랙
    - 
  - N의 부모 노드 P가 레드고, P의 형제 노드 U가 레드
