...
- 만족하지 않는다면 조건을 만족하지 않는 서브트리의 가장 아래 노드 Z를 찾음
- Z로부터 거슬러 올라가 처음으로 조건을 만족시키지 않는 노드를 W라 할 때, W-Z 경로 상에서 회전을 실시
  - 이 때 W의 자식을 Y, 손자를 X라 할 때 X와 Y의 위치 관계에 따라 4가지 회전이 가능함
  - LL, LR, RL, RR
    - 이거 표기법이 책이랑 영문 위키랑 다른데 수정을 해야 하나?
...
- 균형인수가 +2인 노드를(W) 균형인수가 +1인 노드의(X) 오른쪽 자식 노드가 되도록 함
  - 이 때 **X노드에 오른쪽 서브트리가 존재한다면** 해당 서브트리는 **W의 왼쪽 서브트리가 되도록 함**
  - 위 그림엔 안 나왔는데 루트가 25인 서브트리가 있다고 생각해보자
- 실행순서는 다음과 같음
  1. X노드의 오른쪽 서브트리 R을 구함
  2. W의 왼쪽 서브트리를 가리키는 포인터에 R의 루트를 할당
  3. X의 오른쪽 서브트리를 가리키는 포인터에 W를 할당
  4. W의 부모 노드가 X를 가리키게 함
...
- LL과 정확히 반대로 해주면 됨 : W를 X의 왼쪽 자식 노드가 되도록 함
  1. X의 왼쪽 서브트리 L을 구함
  2. W의 오른쪽 서브트리를 가리키는 포인터에 L을 할당
  3. X의 왼쪽 서브트리를 가리키는 포인터에 W를 할당
  4. W의 부모노드가 X를 가리키게 함
...
- Y에 대해 RR회전을 해주고 W에 대해 LL회전을 해줌
...
- Y에 대해 LL회전을 해준 다음 W에 대해 RR회전을 해줌
...
- 경우2) N의 부모 노드 P가 블랙인 경우
  - 모든 조건을 만족하므로 pass
- 경우3) N의 부모 노드 P가 레드고, P의 형제 노드 U가(삼촌 노드) 블랙
  - 회전을 수행함
  - [하는 방법](https://m.blog.naver.com/min-program/221231697752)
- 경우4) N의 부모 노드 P가 레드고, 삼촌 노드 U가 레드
  - N을 블랙으로 바꿔줌
...
[추후보강](https://velog.io/@main_door/%EB%A0%88%EB%93%9C%EB%B8%94%EB%9E%99%ED%8A%B8%EB%A6%AC)
