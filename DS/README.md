...
- 이름부터가 그렇지만 이진 탐색을 위해 고안됨
  ...
  - 단 트리의 균형이 깨지면 최악의 경우 O(n)까지도 증가할 수 있음
    - 정렬된 수열을 순서대로 넣으면 한쪽 방향으로만 노드가 추가됨
...
- 삽입 or 삭제 동작 후 트리가 **AVL의 조건**(`|balance factor| < 2`)을 만족하는지 확인
- 만족하지 않는다면 W로부터 root로 가는 경로상에 **가장 처음으로 위치한 불균형한 노드 Z**를 찾음
...
- **Y에 대해 RR회전**을 해준 다음 **Z에 대해 LL회전**을 해줌
...
- **Y에 대해 LL회전**을 해준 다음 **Z에 대해 RR회전**을 해줌
...
- 정점(vertex)과 간선(edge)을 이용하여 연결 관계를 표현한 것
  - 정점 : 연결의 대상이 되는 개체 또는 위치
  - 간선 : 정점 사이의 연결
- 그래프의 종류
  - 방향 그래프(directed graph) : 간선에 방향정보를 포함하는 그래프
  - 무방향 그래프(undirected graph) : 연결 관계에 방향성이 없는 그래프
  - 완전 그래프(complete graph) : 각각의 정점에서 다른 모든 정점을 연결한 그래프
    - 정점의 수가 같을 때 방향 그래프의 간선의 수는 무방향 그래프의 간선의 수의 2배가 됨
  - 가중치 그래프(weight graph) : 간선에 가중치 정보(거리, 요금 등)을 두어서 구성한 그래프
  - 부분 그래프(sub graph) : 원 그래프의 일부 정점 및 간선으로 이루어진 그래프
- 그래프는 집합으로 표현할 수 있음
  - 정점 집합`V(G)`과 간선 집합 `E(G)`로 나누어서 표현 가능
  - 무방향 그래프의 간선은 `(A, B)`, 방향 그래프의 간선은 `<A, B>`로 나타냄



그래프의 ADT

- 간선의 가중치 부여, 삭제, 방향성 여부 선택 등은 그래프 구현에서 그다지 중요하지 않기 때문에 생략
  - 왜?
- `void GraphInit(UALGraph * pg, int nv);`
  - 그래프의 초기화를 진행
  - 정점의 갯수를 두 번째 인자로 전달함
- `void GraphDestroy(UALGraph * pg);`
  - 그래프 초기화 과정에서 할당한 리소스를 반환함
- `void AddEdge(UALGraph * pg, int fromV, int toV);`
  - 매개변수 fromV와 toV로 전달된 정점을 연결하는 간선을 그래프에 추가함
...
- **인접 행렬**(adjacent matrix) 기반 그래프
  - 정방 행렬(2차원 배열)을 활용
  - 두 정점이 연결되어 있으면 1, 아니면 0으로 표시하는 행렬을 작성함
    - 무방향 그래프의 경우 대각선을 기준으로 대칭을 이룸
- **인접 리스트**(adjacent list) 기반 그래프
  - 연결 리스트를 활용
  - 각각의 정점이 자신과 연결된 정점의 정보를 담기 위해 하나의 연결 리스트를 가짐
...
- 데이터를 저장할 위치(인덱스)를 **간단한 연산**으로 구하는 것
  - 이 때 사용되는 함수를 **해시 함수**라고 함
  - 해시 함수는 넓은 범위의 Key를 고정된(좁은 범위의) Key 값으로 변환해 줌
...
- 함수 값의 **충돌이 최소화**되는지?
  ...
  - 사용할 키의 *모든 정보*를 이용하여 해싱하는지?
    - 많은 수의 데이터를 조합해야 더 다양한 값의 생성을 기대할 수 있음
- 연산이 빠른지?
...
- **키의 특성과 저장공간의 크기를 고려**하여 적절한 방법을 선택해야 함

- Digit Selection

  - Key에서 **일부분만을 추출**하여 해시 값을 생성하는 방법
- 키의 특정 위치에서 중복의 비율이 높거나 아예 공통으로 들어가는 값이 있는 경우에 사용
  - 예 : 초등학교 같은 반 학생들의 생년월일(연도가 전부 중복되므로 제거)
- 유사한 방법으로 **Bit Selection**이 있음
    - Key의 비트 열에서 일부를 추출하여 조합
...
- 기타
  ...
  - Digit Folding
    - 종이를 접듯이 숫자를 겹치게 하여 거한 결과를 해시 값으로 결정
    - 예) 273419를 3등분하여 27 + 34 + 19의 결과인 80을 해시 값으로 함
  - 키를 제곱하여 그 중 일부를 추출하는 방법
  - 덧셈 대신 XOR 연산을 이용하여 Folding 하는 방법
  - 둘 이상의 방법을 조합하는 방법 등등
...
- 서로 다른 객체 X와 Y에 대해 해시함수 연산 결과가 동일한 것
  - **perfect hash function** : `X.hashCode() == Y.hashCode()` 이면  X.equals(Y)는 참
  - 1대 1 함수면 perfect hash function이다
...
- Closed addressing이라고도 함 : 충돌하더라도 **해당 자리에 저장**한다는 뜻
- 충돌이 발생한 자리에 어떻게 저장하는가?
  - 2차원 배열을 구성해 하나의 해시 값에 대해 다수의 bucket을 둠
    - 비효율적이기 때문에 사용하지 않음 : 메모리 낭비 + 최대 충돌 수에 제한 존재
  - **연결 리스트로 연결**하는 방법
    - 트리로 연결할 수도 있는데 이건 하나의 버킷에 데이터가 많이 들어오는 경우에 그렇게 함
    - Java 8 HashMap에서는 하나의 해시 버킷에 8개의 키-값 쌍이 모이면 트리로 변경함
- 시간 복잡도
  - 잘 구현되면 O(1), 최악의 경우 O(n): 모든 삽입에 대해 해시 충돌이 발생할 경우
- **해시 테이블** 구조의 원형임
...
- 각각의 Bucket을 노드로 하여 연결 리스트를 만드는 방법

  - 구조체 Bucket에 key, value 뿐만 아니라 다음 bucket을 가리키는 포인터 변수의 추가가 필요함
  - 기존에 구현되어 있는 List 자료구조를 활용하지 않고 새로 작성하는 경우에 사용

- 연결 리스트의 노드에 bucket의 주소를 저장하는 방법

  - 연결 **리스트 관련 코드와 해시 테이블 관련 코드의 구분이 용이**하기 때문에 추천되는 방법

  - 노드에 저장할 데이터의 타입을 결정하는 typedef 선언문의 추가가 필요함

  - ```c
    typedef Bucket * Data;
    
    typedef struct __node{
        Data data;				// Bucket을 가리키는 포인터를 담음
        struct __node * next;
    } Node;
    ```

- Bucket이 노드의 멤버가 되도록 하는 방법

  - ```c
    typedef Bucket Data;
    
    typedef struct __node{
        Data data;				// 포인터가 아니라 Bucket을 직접 담음
        struct __node * next;
    } Node;
    ```
...
- Probing
  ...
  - Linear Probing : 충돌이 발생할 경우 **옆 bucket이 비었는지 반복적으로 탐색**

    - $$
      f(k)+1 → f(k)+2 → f(k)+3 → ...
      $$

    - 클러스터링 현상이 발생한다는 단점이 있음

  - Quadratic Probing : 2차 함수를 이용해서 탐색

    - $$
      f(k)+1^2 → f(k)+2^2 → f(k)+3^2 → ...
      $$

    - 붙어있는 빈 공간이 아닌 **멀리 있는 빈 공간을 찾음**으로써 클러스터링을 극복함

    - 동일한 해시 값에 대해 접근하는 위치가 늘 동일하다는 단점이 있음

  - 주의할 점 : Bucket의 상태에 **DELETED를 표현해야 함**

    - 충돌이 일어난 bucket의 데이터를 삭제한 후 해당 위치의 인덱스를 해시 값으로 갖는 데이터를 탐색하려고 하면 데이터가 존재하지 않는다고 판단하여 탐색이 종료됨
    - 선형/이차 조사법을 적용 했다면 탐색 과정에서도 충돌을 고려하여야 함

- Double hash

  - 충돌이 발생하면 다른 해시 함수를 이용해 저장할 bucket의 위치를 결정함
  - 1차 해시 함수
    - 키를 근거로 저장 위치를 결정하기 위한 것
    - 예 : `h1(k) = k % 15`
  - 2차 해시 함수
    - 충돌 발생시 **몇 칸 뒤를 살필지** 결정하기 위한 것
    - 예 : `h2(k) = 1 + (k % c)`
    - 일반적으로 c에는 1차 해시 값의 제수(15)보다 작은 소수를 사용함
    - 2차 해시 함수로 얻은 값에서도 충돌이 발생하면 probing과 마찬가지로 **반복적으로 공간을 찾음**
...
- **탐색에 O(1)**의 시간복잡도를 갖는 자료구조
  - 데이터의 mapping을 통해 단번에 탐색을 수행함
  - Map, Dictionary라고도 함
- 저장되는 데이터는 key - value pair를 이룸
  - **key는 데이터를 찾는 도구**로, 키를 이용해 단번에 데이터를 찾을 수 있어야 함
    - 키의 값은 저장위치
    - 키의 범위가 너무 크면 매우 큰 배열이 필요함 : **해시 함수의 도입**
  - key는 중복되어선 안 되고, NULL 값을 가질 수 없음
    - 이 조건을 만족하지 않으면 테이블이라 할 수 없음
...
3. 해당 인덱스에 데이터가 없다면 저장
4. 이미 데이터가 있다면 **연결 리스트**로 연결
