...
- 대규모 시스템에 부적합함
  ...
  - **CORS**(Cross-Origin Resource Sharing)
    - 세션 관리에 사용되는 **쿠키**를 여러 도메인에서 관리하는 것은 번거로움
...
- 추가적인 HTTP header를 사용해서 **애플리케이션이 다른 origin의 리소스에 접근**할 수 있도록 하는 메커니즘
- CORS의 필요성
  - 외부 공격 차단
...
-  CORS 동작 방식
  ...
  - 브라우저가 이 헤더를 보고 요청할 수 있다면 리소스 전송 허용하고 불가능하다면 에러 발생시킴
...
- Uniform Resource **Locator** : 자원의 위치를 나타내며 URI에 포함되는 개념임
  - URI가 자원의 식별자라면 URL은 자원이 어디에 위치하는지까지만 나타냄
  - 웹상 뿐만 아니라 컴퓨터 네트워크상의 자원은 모두 나타낼 수 있음
    - 일반적으로는 사이트 도메인을 의미함
- URI 설계 원칙
  - 가독성 관련
...
- Self-descriptiveness
  - 메시지만 보고도 요청하는 데이터가 어떻게 처리되어야 하는지 이해할 수 있어야 함
