...
- `instanceof` 키워드
  ...
  - 다운 캐스팅을 하기 전에는 반드시 instanceof로 체크를 할 것
...
- **추상 메서드를 포함한 **클래스
  ...
  - 추상 메서드란 구현부 없이 선언만 된 메서드: 앞에 abstract를 붙여서 구분함
  ...
  - abstract를 추가하면 추상 메서드가 되기 때문에, 이제는 클래스에도 abstract를 추가하라고 함
...
- 템플릿 메서드
  - 추상 메서드와 구현된 메서드를 이용하여 코드의 흐름을 정의하는 메서드
...
- 클래스는 왜 다중 상속이 안 될까?
  ...
  - 인터페이스는 구현이 안 되어있기 때문에 다중 상속(정확히는 구현)이 가능한 것
