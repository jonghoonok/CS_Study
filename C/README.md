...
- 장점
  ...
  - 이식성이 높고 유연함
    - **CPU 종류에 상관 없이 실행이 가능**함: 어셈블리어는 하드웨어 의존적
    - 기본적으로 모든 것을 만들 수 있음
  - 성능이 좋음
...
- 전역변수
  - 프로그램의 어디서든 접근 가능, main이 실행되기 전에 **Data 영역 메모리에 할당**
    - 중괄호 내에 선언되지 않음
    - 프로그램 시작과 동시에 할당되어 종료시까지 존재
    - 초기화하지 않으면 0으로 초기화 됨
  - 프로그램이 커지면 전역변수로 인해 프로그램이 복잡해질 수 있음: **사용 자제**
    - 전역변수의 변경은 다른 함수의 변경을 요구함
- 지역변수
  - 특정 블록에서만 접근 가능, 함수가 **실행될 때마다 Stack 영역에 기록**
- 정적변수
  - 특정 블록에서만 접근 가능: 프로그램이 실행될 때 메모리 Data 영역에 할당되고, 종료되면 해제
...
- register변수
  - 메인 메모리 대신 레지스터를 사용
    - 레지스터는 한정되어 있으므로 실제로 레지스터가 사용될지는 모름: 컴파일러가 저장 여부 결정
...
![memory](http://www.tcpschool.com/lectures/img_c_memory_structure.png)

- 코드(code) 영역
  - 실행할 프로그램의 코드가 저장되는 영역, 텍스트(code) 영역이라고도 부름
  - CPU는 코드 영역에 저장된 명령어를 하나씩 가져가서 처리
- 데이터(data) 영역
  - 프로그램의 전역 변수와 정적(static) 변수가 저장되는 영역
  - 데이터 영역은 프로세스의 시작과 함께 할당되며, **프로그램이 종료되면 소멸**
- 스택(stack) 영역
...
- `int num;` 
  ...
  - num: 그 공간의 이름을 num이라 **선언**함
    ...
    - 컴파일러에 따라 다른데(MinGW는 아니었음) 기본적으로는 그러함
- `num = 1;`
...
- 자료형 별 서식문자
  - ![format specifiers](https://lh3.googleusercontent.com/proxy/qP6Cl92rUwcrjWwgxTOccRsMwyi7xE225dCePPGwItDR3OCWziMmFIme2wY9BT20p-5latzPyqCqak7fQUBL6_kJhgQ8UZXFNprzfs_vG-v4nw-4i_Y0CZPT)
...
- ASCII Code
  ...
  - A는 65, a는 97이다
    - A: 01000001
    - a: 01100001
    - 소문자는 대문자 아스키 코드에 32를 더하면 됨(왼쪽에서 3번째 비트)
...
- 이름이 있는 상수: **symbolic**
  - **변수 선언 시 const만 추가**하면 되고, **선언과 동시에 초기화**해야 함: `const int MAX = 100;`
...
- **문자열 리터럴**은 메모리 공간에 저장된 후 주소값이 반환됨
  - String constant: 큰 따옴표에 묶인 문자열, 변경할 수 없음
    ...
    - 이 경우엔 배열을 먼저 만들어서 문자를 하나씩 넣은 것으로 문자열 상수가 아님
    - 문자열 배열 변수의 내용물은 **Heap**에, 문자열 리터럴은 **정적영역**에 저장됨
    - 문자열 리터럴을 변경하려고 하면 **Segmentation fault** 발생
...
- 호출된 이후에 **전달되는 인자와 매개변수는 별개**가 됨
  - 함수 내에서 **복사된 값을 갖는 변수가 새롭게 생성**된다고 이해하면 됨
    - 생성된 변수는 스택 내 다른 공간에 저장!
    - 함수 별로 스택의 공간을 할당하기 때문: 스택 프레임
...
- `sizeof(arr) / sizeof(arr[0])`
  - **2차원 배열의 행 길이를 계산**할 때 흔히 사용되는 방법
- `arr[i] == *(arr + i)`
  - `arr[2][1] = 4;`는
  - `(*(arr+2))[1] = 4;`로 대체 가능
...
- 함수 포인터의 선언: 반환형과 매개변수 선언의 정보가 필요
  ...
  - 주의: **함수 포인터는 괄호로 감싸야 함**
    - `double (*fp)(int);` 는 int를 받아 double을 반환하는 *"함수의 **포인터**"*
